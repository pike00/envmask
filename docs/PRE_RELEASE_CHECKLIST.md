...
- [ ] Update version in `src/envguard/__init__.py`
- [ ] Update `CHANGELOG.md` with release notes
- [ ] Commit changes: `git add -A && git commit -m "Release vX.Y.Z"`
- [ ] Build distribution: `uv build` (or `python -m build`)
- [ ] Verify wheel: `unzip -l dist/envguard-*.whl | grep -E '(masker|cli|hook)'`
- [ ] Verify source: `tar tzf dist/envguard-*.tar.gz | grep -E '(masker|cli|hook)'`

## Installation Testing

- [ ] Create clean test environment: `python -m venv /tmp/test-env-X`
- [ ] Activate: `source /tmp/test-env-X/bin/activate`
- [ ] Install from wheel: `pip install dist/envguard-*.whl`
- [ ] Test CLI: `envguard --version` (should show correct version)
- [ ] Test CLI help: `envguard --help`
- [ ] Test module: `python -c "from envguard import mask_value; print(mask_value('test'))"` (should print `tes...`)
- [ ] Test hook: `echo '{"tool_input":{"file_path":".env"}}' | python -m envguard.hook` (should output JSON)
...
- [ ] Check PyPI: https://pypi.org/project/envguard/
...
- [ ] Install from PyPI: `pip install envguard==X.Y.Z`
- [ ] Verify CLI works: `envguard --version`
- [ ] Verify hook works: `echo '{"tool_input":{"file_path":".env"}}' | python -m envguard.hook`
- [ ] Check package stats: https://pypi.org/project/envguard/#history
